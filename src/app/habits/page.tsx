...
import { Plus, Sparkles, Coins, Check, Pencil, Trash2, Loader2, Clock } from "lucide-react";
...
export default function HabitsPage() {
  ...
  const [form, setForm] = useState({ title: "", description: "", xpReward: "25", coinReward: "10", reminderTime: "" });
  const [saving, setSaving] = useState(false);
  const [xpToast, setXpToast] = useState<{ xp: number; coins: number; show: boolean }>({ xp: 0, coins: 0, show: false });
  ...
  const openCreateDialog = () => {
    ...
    setForm({ title: "", description: "", xpReward: "25", coinReward: "10", reminderTime: "" });
    setDialogOpen(true);
  };
        ...
        setHabits((prev) =>
          prev.map((h) =>
            h.id === editingHabit.id
              ? { ...h, title: form.title, description: form.description, xpReward: Number(form.xpReward) || 25, coinReward: Number(form.coinReward) || 10, reminderTime: reminderMinutes ? Number(reminderMinutes) : null }
              : h
          )
        );
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#A1A1A1]">{completedCount} of {habits.length} completed today</p>
        </div>
        {/* Desktop add button */}
      ...
      {habits.length > 0 && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#121212] p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-bold text-[#FF7A00]">{Math.round((completedCount / habits.length) * 100)}%</span>
          </div>
          <Progress
      ...
      <div className="space-y-3">
        {habits.length === 0 ? (
          <div className="rounded-2xl border border-white/[0.06] bg-[#121212] p-12 text-center">
            <p className="text-[#A1A1A1] text-sm mb-4">No habits yet. Create your first one!</p>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn(
                    "font-semibold",
                    habit.completedToday ? "text-[#A1A1A1] line-through" : "text-white"
                  )}>
                    {habit.title}
                  </p>
                  {habit.description && (
                    <p className="text-sm text-[#A1A1A1] truncate">{habit.description}</p>
                  )}
                </div>
                ...
                <div className="flex flex-col items-end gap-2 shrink-0">
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1 rounded-lg bg-[#FF7A00]/10 px-2 py-1">
                      ...
                      <span className="text-xs font-semibold text-[#FF7A00]">{habit.xpReward}</span>
                    </div>
                    <div className="flex items-center gap-1 rounded-lg bg-[#EAB308]/10 px-2 py-1">
                      <Coins className="h-3 w-3 text-[#EAB308]" />
                      <span className="text-xs font-semibold text-[#EAB308]">{habit.coinReward}</span>
                    </div>
                  </div>
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => openEditDialog(habit)} className="p-1.5 rounded-lg hover:bg-white/5 text-[#A1A1A1] hover:text-white transition-colors">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => handleDelete(habit.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-[#A1A1A1] hover:text-red-400 transition-colors">
      ...
      <button
        ...
        className="fixed bottom-20 right-4 z-40 flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[#FF7A00] to-[#FF9F3F] text-white shadow-xl shadow-[#FF7A00]/30 transition-transform hover:scale-105 active:scale-95 sm:hidden"
      >
        ...
        <DialogContent>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-[10px] text-[#A1A1A1]">Leave empty for no reminder</p>
            </div>
      ...
      {/* XP Toast */}
      <XpToast xp={xpToast.xp} coins={xpToast.coins} show={xpToast.show} onComplete={() => setXpToast(prev => ({ ...prev, show: false }))} />
    </div>
  );
}
