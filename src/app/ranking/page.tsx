...
const podiumColors = {
  1: { bg: "bg-[#FFD700]/10", border: "border-[#FFD700]/30", text: "text-[#FFD700]", ring: "ring-[#FFD700]/40", shadow: "shadow-[#FFD700]/10" },
  2: { bg: "bg-[#C0C0C0]/10", border: "border-[#C0C0C0]/30", text: "text-[#C0C0C0]", ring: "ring-[#C0C0C0]/40", shadow: "shadow-[#C0C0C0]/10" },
  3: { bg: "bg-[#CD7F32]/10", border: "border-[#CD7F32]/30", text: "text-[#CD7F32]", ring: "ring-[#CD7F32]/40", shadow: "shadow-[#CD7F32]/10" },
} as const;
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-[#A1A1A1]">Top players this week based on XP earned</p>
      </div>
      ...
      {userPosition && (
        <div className="rounded-2xl border border-[#FF7A00]/20 bg-[#FF7A00]/[0.06] p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-white">Your Position: <span className="text-[#FF7A00]">#{userPosition.position}</span></p>
            <p className="text-xs text-[#A1A1A1]">{userPosition.xpEarned.toLocaleString()} XP earned this week</p>
          </div>
        </div>
      )}
      ...
      {ranking.length === 0 && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#121212] p-12 text-center">
          <p className="text-[#A1A1A1] text-sm">No ranking data yet. Start completing habits to climb the leaderboard!</p>
        </div>
      )}
          ...
          {rest.map((entry, i) => (
            <div
              ...
              className={cn(
                ...
                entry.isCurrentUser ? "bg-[#FF7A00]/[0.06]" : "hover:bg-white/[0.02]"
              )}
            ...
            >
              ...
              {/* Avatar */}
              <Avatar className={cn("h-10 w-10", entry.isCurrentUser && "ring-2 ring-[#FF7A00]/40")}>
              ...
              <div className="flex-1 min-w-0">
                <p className={cn(
                  ...
                  {entry.name}
                  {entry.isCurrentUser && <span className="text-[#A1A1A1] font-normal ml-1">(you)</span>}
                </p>
              ...
              <div className="text-right shrink-0">
                <p className="text-sm font-bold text-[#FF7A00]">{entry.xpEarned.toLocaleString()}</p>
...
function PodiumCard({ entry, isFirst = false }: {
  entry: RankingEntry;
  isFirst?: boolean;
}) {
  ...
  return (
    ...
    >
      {isFirst && <Crown className="h-6 w-6 text-[#FFD700] mb-2 animate-glow-pulse" />}

      <div className={cn(
        "flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold mb-2",
        pos === 1 ? "bg-[#FFD700]/20 text-[#FFD700]" :
        pos === 2 ? "bg-[#C0C0C0]/20 text-[#C0C0C0]" :
      ...
      <Avatar className={cn(
        ...
        colors.ring, colors.shadow,
        isFirst ? "h-16 w-16" : "h-12 w-12"
      )}>
        <AvatarFallback className={cn("text-sm", isFirst && "text-lg")}>{getInitials(entry.name)}</AvatarFallback>
      </Avatar>

      <p className={cn("font-semibold text-white text-center truncate w-full", isFirst ? "text-sm" : "text-xs")}>
        {entry.name}
      </p>
